...
def scan_cipher_ssl(list_url):
    for url in list_url:
        ...
        if list(set(prohibited_cipher_list) & set(data)):
                ...
                slack.chat.post_message(slack_channel,'@mfranz  @sgoswami BAD CIPHER FOUND!! For -> '+str(url)+'Contains Vulnerable Cipher : '+str((list(set(prohibited_cipher_list) & set(data)))),username='SSL_Lab_Bot',link_names='2')
        else:
